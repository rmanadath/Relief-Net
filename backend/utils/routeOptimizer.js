...
/**
 * Nearest Neighbor Algorithm for Route Optimization (TSP Variant)
 * 
 * Algorithm Type: Greedy heuristic for Traveling Salesman Problem
 * Time Complexity: O(n²) where n is the number of requests
 * Space Complexity: O(n) for visited set and result array
 * 
 * This is a greedy algorithm that finds a good (not always optimal) solution.
 * It works by always selecting the nearest unvisited location, which provides
 * a reasonable approximation of the optimal route.
 * 
 * Algorithm Steps:
 * 1. Start at the volunteer's location
 * 2. Find the nearest unvisited request
 * 3. Add it to the route and mark as visited
 * 4. Move to that location
 * 5. Repeat until all requests are visited
 * 
 * @param {Array} requests - Array of requests with lat/lng coordinates
 * @param {Object} startLocation - Starting location {lat, lng}
 * @returns {Array} Optimized order of requests with distanceFromPrevious
 */
